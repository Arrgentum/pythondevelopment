...
class App(Application):
    def canvas_click(self, event):
        p = self.canvas.find_overlapping(event.x, event.y, event.x + 1, event.y + 1)
        if p:
           return
        self.current_start = event.x, event.y
        self.current_end = event.x + 10, event.y + 10
        self.current_obj = self.canvas.create_oval(*self.current_start, *self.current_end, fill="white")
        self.canvas.tag_bind(self.current_obj, '<Button-1>', partial(self.oval_click, self.current_obj))
    ...
    def object_movement(self, tag, event):
        ...
        diff_x, diff_y = event.x - start_x, vent.y - start_y
